...
final class Jackson3Obfuscator extends JSONObfuscator {

    private static final Logger LOGGER = LoggerFactory.getLogger(Jackson3Obfuscator.class);
    ...
    void obfuscateText(Reader input, Source source, int start, int end, LimitAppendable destination) throws IOException {
        ...
        try (JsonParser jsonParser = jsonFactory.createParser(ObjectReadContext.empty(), input)) {
